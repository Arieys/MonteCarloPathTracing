...
void scene_data::loadModel(string const& path)
{
	// 通过ASSIMP读文件
...
void scene_data::processNode(aiNode* node, const aiScene* scene)
{
	// 处理位于当前节点的每个网格
...
void scene_data::processMesh(aiMesh* mesh, const aiScene* scene)
{
	// 临时数据
...
// 所需信息作为Texture结构返回。
int scene_data::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{   // 如果尚未加载纹理，请加载它
			...
			std::cout << "Read in texture : " << str.C_Str() << " From " << directory << std::endl;
			//cv::imshow("window", texture.img);
			//cv::waitKey(100);
